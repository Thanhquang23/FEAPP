...
function deleteCustomer() {
  ...
  async function deleteCustomer(id) {
    const apiUrl = `https://beapp-nine.vercel.app/api/services/${id}`;
...
const apiUrl = "https://beapp-nine.vercel.app/api/services";
const authenticationHeader = {
  "Authorization": authorization,
};
...
function validateForm() {
  if (check === 0) {
    ...
    if (nameInput.value.trim() === '') {
      nameError.textContent = 'Vui lòng điền tên dịch vụ';
    } else {
    ...
    // Kiểm tra tính hợp lệ của trường "Price"
    const priceInput = document.getElementById('price');
    const priceError = document.getElementById('price-error');
    if (priceInput.value.trim() === '') {
      priceError.textContent = 'Vui lòng điền giá dịch vụ';
    } else {
