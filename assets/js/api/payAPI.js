...
function find() {
  ...
  if (info.length > 0) {
    ...
    const apiUrl = "https://beapp-nine.vercel.app/api/customers/" + info; // Thay thế "your-api-url" bằng URL tương ứng trong ứng dụng của bạn
    const authenticationHeader = {
      "Authorization": authorization,
    };
        ...
        createData(apiUrl, authorization, data)
          .then((responses) => {
            cart.forEach((service) => {
              ...
              const apiUrl = "https://beapp-nine.vercel.app/api/orderDetails";
              const data = {
                order: responses,
